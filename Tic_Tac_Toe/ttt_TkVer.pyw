...
class App(Frame): # the whole game screen is a big Frame
	...
	def __init__(self, master = None):
		...
		self.list = [""]*9 # Another way is to define an empty list to store labels
		...
		for AllBut in ("123","456","789"):
			...
			for OneBut in AllBut:
				...
				# self.list[int(OneBut) - 1] = label(self, self.display[int(OneBut) - 1], x[0], y[0])
				# self.list[int(OneBut) - 1].bind('<Button-1>', lambda event, e = int(OneBut): self.callback(e))
	...
	def callback(self, number):
		...
		if self.display[number - 1].get() == "": # if this specific button is empty
			...
			if list.count("") % 2 == 0:
				...
				# self.list[int(number) - 1].config(foreground = ["red" if (self.display[int(number) - 1].get() == "O") else "black"][0])
			else:
